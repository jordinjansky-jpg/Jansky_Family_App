...
export function initNavMore(sheetMount, getTheme, personOpts, familyOpts, onApply, opts = {}) {
  ...
  const ALL_PAGES = {
    ...
    activities: { id: 'activities', label: 'Activities', href: 'activities.html', icon: _svg('<path d="M22 12h-4l-3 9L9 3l-3 9H2"></path>') },
  };
...
function renderTimeOfDayPill(slot) {
  ...
  // Sun center at (7,7): x+y=14, distance from divider (x+y=24) = 10/√2 ≈ 7.07 units.
  // Moon center ≈ (16,18): x+y=34, distance = 10/√2 ≈ 7.07 units. Both equidistant from /.
  const anytimeIcon = `<svg viewBox="0 0 24 24" fill="none" stroke-linecap="round" stroke-linejoin="round" width="20" height="20"><g class="anytime-sun" stroke="currentColor" stroke-width="1.5"><circle cx="7" cy="7" r="2"/><line x1="7" y1="3.5" x2="7" y2="5"/><line x1="7" y1="9" x2="7" y2="10.5"/><line x1="3.5" y1="7" x2="5" y2="7"/><line x1="9" y1="7" x2="10.5" y2="7"/><line x1="5.1" y1="5.1" x2="6.1" y2="6.1"/><line x1="7.9" y1="7.9" x2="8.9" y2="8.9"/><line x1="5.1" y1="8.9" x2="6.1" y2="7.9"/><line x1="7.9" y1="6.1" x2="8.9" y2="5.1"/></g><line class="anytime-divider" x1="20" y1="4" x2="4" y2="20" stroke="currentColor" stroke-width="1"/><g class="anytime-moon" stroke="currentColor" stroke-width="1.5"><path d="M20 17.5A4 4 0 1 1 15.5 14 3.2 3.2 0 0 0 20 17.5z"/></g></svg>`;
...
export function renderTaskCard(options) {
  ...
  const catIcon = showIcon ? renderCategoryIcon(category, { size: 15 }) : '';
...
export function renderBanner(variant, { title, message, action, bodyClickable = false } = {}) {
  const iconMap = {
    ...
    approvals:  `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>`,
  };
...
export function renderTrackerFilterSheet(cats, activeCategory, activeStatus) {
  ...
  const catChips = [
    ...
    ...catEntries.map(([key, cat]) => {
      const label = (cat.label || cat.name || key).trim();
      return `<button class="chip chip--selectable${activeCategory === key ? ' chip--active' : ''}" data-filter-cat="${esc(key)}" type="button">${renderCategoryIcon(cat, { size: 14 })} ${esc(label)}</button>`;
    }),
  ].join('');
...
export function renderTaskDetailSheet(options) {
  ...
  const catIcon    = renderCategoryIcon(category, { size: 17 });
  const ownerColor = person?.color || 'var(--text-faint)';
