...
function _getAccentBright(hex) {
  const m = hex.replace('#', '');
  const r = parseInt(m.substring(0, 2), 16) / 255;
  const g = parseInt(m.substring(2, 4), 16) / 255;
  const b = parseInt(m.substring(4, 6), 16) / 255;
  const lum = 0.2126 * r + 0.7152 * g + 0.0722 * b;
  const mixAmount = lum < 0.4 ? 50 : lum < 0.6 ? 35 : 20;
  return `color-mix(in srgb, ${hex} ${100 - mixAmount}%, #ffffff)`;
}
...
/**
 * Apply task card display preferences from settings to <html> data attributes.
...
export function getColorPalette() {
  return [
    '#e53935', '#e91e63', '#f06292', '#f48fb1', '#9c27b0', '#ab47bc', '#7e57c2', '#9575cd',
    '#3f51b5', '#5c6bc0', '#1e88e5', '#42a5f5', '#0097a7', '#00acc1', '#009688', '#26a69a',
    '#2e7d32', '#43a047', '#66bb6a', '#9ccc65', '#f9a825', '#fdd835', '#ffa726', '#ff8f00',
    '#ff7043', '#f4511e', '#e64a19', '#b71c1c', '#795548', '#8d6e63', '#607d8b', '#78909c',
    '#ef9a9a', '#ce93d8', '#90caf9', '#80cbc4', '#a5d6a7', '#fff9c4', '#ffe082', '#c5e1a5',
  ];
}
